...
def _get_exception_code(status_code: int) -> int:
    """
    Get the return status code (the available status code is based on RFC definition)
    ...
    `IANA Status Code Registration <https://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml>`__
...
async def _validation_exception_handler(request: Request, exc: RequestValidationError | ValidationError):
    ...
    for error in exc.errors():
        # Use custom error messages in non-en-US languages
    ...
    if error.get('type') == 'json_invalid':
        ...
        message = f'{field} {error_msg}，Entry：{error_input}' if settings.ENVIRONMENT == 'dev' else error_msg
...
def register_exception(app: FastAPI):
    ...
    async def fastapi_validation_exception_handler(request: Request, exc: RequestValidationError):
        """
        FastAPI data verification exception handling
    ...
    async def pydantic_validation_exception_handler(request: Request, exc: ValidationError):
        """
        Pydantic data verification exception handling
