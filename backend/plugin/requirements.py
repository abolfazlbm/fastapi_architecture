...
def _requirements_installed(requirements_file: Path) -> bool:  # noqa: C901
    """Check whether requirements and its extras sub-dependencies are installed"""
    requirements = []
    for line in requirements_file.read_text(encoding='utf-8').splitlines():
        ...
        try:
            ...
            raise PluginInstallError(f'depends on {line} format error: {e!s}') from e
    ...
    def requirement_satisfied(requirement: Requirement, active_extras: frozenset[str] = frozenset({''})) -> bool:
        ...
        for dependency_line in dist.requires or []:
            try:
                ...
                raise PluginInstallError(f'dependency metadata {dependency_line} format error: {e!s}') from e
