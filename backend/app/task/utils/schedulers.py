...
# The lock detection period should be less than the planned lock time
...
class ModelEntry(ScheduleEntry):
    ...
    def __init__(self, model: TaskScheduler, app=None):
        ...
        try:
            ...
            logger.error(f'Disable task with scheduled empty {self.name}, details: {e}')
        ...
        try:
            ...
            logger.error(f'Disable task with parameter error: {self.name}; error: {str(exc)}')
            asyncio.create_task(self._disable(model))
    ...
    def is_due(self) -> tuple[bool, int | float]:
        ...
        if self.model.one_off and self.model.enabled and self.model.total_run_count > 0:
            ...
            return schedules.schedstate(is_due=False, next=1000000000)  # High latency, avoid rechecking
    ...
    async def save(self, fields: tuple = ()):
        ...
        async with async_db_session.begin() as db:
            ...
            if task:
                ...
                logger.warning(f'task {self.model.name} does not exist, skip update')
    ...
    async def to_model_schedule(name: str, task: str, schedule: schedules.schedule | TzAwareCrontab):
        ...
        async with async_db_session() as db:
            if isinstance(schedule, schedules.schedule):
                ...
                raise errors.NotFoundError(msg=f'Plan types that are not supported yet：{schedule}')
...
class DatabaseScheduler(Scheduler):
    ...
    def reserve(self, entry):
        ...
        # entries need to be stored by name because the entry may change
        ...
        try:
            while self._dirty:
                ...
                try:
                    ...
                    logger.debug(f'Save task {name} latest status to database')
                    _tried.add(name)
                except KeyError as e:
                    logger.error(f'Save task {name} Latest status failed: {e} ')
                    _failed.add(name)
        except DatabaseError as e:
            logger.exception('Domain error occurred while synchronizing: %r', e)
        except InterfaceError as e:
            logger.warning(f'DatabaseScheduler InterfaceError: {str(e)}, wait for the next call to try again...')
        finally:
            # Please try again later (only for failed ones)
            self._dirty |= _failed
    ...
    def update_from_dict(self, beat_dict: dict):
        ...
        for name, entry_fields in beat_dict.items():
            try:
                ...
                logger.error(f'Add task {name} to database failed')
                raise e
    ...
    def schedule(self) -> dict[str, ModelEntry]:
        ...
        if update:
            ...
            # The plan has been changed, invalidating the heap in Scheduler.tick
            if not initial:
                self._heap = []
